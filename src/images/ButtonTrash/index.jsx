...
function ButtonTrash ({listTransactions, setListTransactions, setIsTotalMoneyActive}) {

    function excludeTransaction(event) {
        const description = event.target.parentElement.parentElement.querySelector("p").innerText;
        const newList = listTransactions.filter(element => element.description !== description);
        if(listTransactions.length === 1) setIsTotalMoneyActive(false);
        setListTransactions(newList);
    }

    return (
        <svg onClick={excludeTransaction} className="buttonTrashImg" width="22" height="22" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
        <rect width="22" height="22" rx="2.46154" transform="matrix(-1 0 0 1 22 0)" fill=""/>
